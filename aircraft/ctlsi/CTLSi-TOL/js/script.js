...
/* ============================================================
   CTLSi Takeoff & Landing Performance
   AeroJones CTLSi (Rotax 912 iS Sport)
   ------------------------------------------------------------
   起飞 / 着陆性能计算器（单页面双 Tab）
   全部性能数据 / 修正系数来自 POH §5
   ------------------------------------------------------------
...
/* ----- 顺风 / 顶风修正 ----- */
function tailwindFactor(tw_ms, kind) {
  if (tw_ms <= 0) return 1;
  const fiveKtMs = 5 * MS_PER_KT;
  const increaseAtFiveKt = kind === 'distance' ? 0.25 : 0.20;
  return 1 + increaseAtFiveKt * (tw_ms / fiveKtMs);
}
function headwindFactor() {
  // Rev.20 §5.2.1/§5.2.3 does not publish a headwind distance credit.
  return 1;
}

/* ----- 坡度修正 ----- */
function slopeRuleFactor(slope_pct, atTwoPct, atFourPct) {
  const magnitude = Math.abs(slope_pct);
  const increase = magnitude <= 2
    ? atTwoPct * magnitude / 2
    : atTwoPct + (atFourPct - atTwoPct) * Math.min(magnitude - 2, 2) / 2;
  return slope_pct < 0 ? Math.max(0, 1 - increase) : 1 + increase;
}
function slopeFactorRoll(slope_pct) { return slopeRuleFactor(slope_pct, 0.10, 0.14); }
function slopeFactorDist(slope_pct) { return slopeRuleFactor(slope_pct, 0.10, 0.12); }

/* ----- 表面修正 ----- */
function surfaceFactors(surface) {
  const map = {
    paved_dry:   { roll: 1.00, dist: 1.00 },
    paved_wet:   { roll: 1.00, dist: 1.00 },
    grass_short: { roll: 1.00, dist: 1.00 },
...
function calcTakeoff() {
  ...
  const rollWindF = wc.tailwind > 0 ? tailwindFactor(wc.tailwind, 'roll') : headwindFactor(wc.headwind);
  const distWindF = wc.tailwind > 0 ? tailwindFactor(wc.tailwind, 'distance') : headwindFactor(wc.headwind);
  const rollCorrected = base.roll * rollWindF * slopeFactorRoll(slope) * surfF.roll;
  const distCorrected = base.dist * distWindF * slopeFactorDist(slope) * surfF.dist;
  ...
  function firstRun() {
    ...
    try {
      ...
      const tow = plan && plan.weightBalance
        ...
        : parseFloat(localStorage.getItem('ctlsi_tow'));
      ...
      const gop = compatibleFuelPlan
        ...
        : (plan ? NaN : parseFloat(localStorage.getItem('ctlsi_ground_op_kg')));
      const trp = compatibleFuelPlan
        ? plan.fuelPlan.tripKg
        : (plan ? NaN : parseFloat(localStorage.getItem('ctlsi_trip_kg')));
