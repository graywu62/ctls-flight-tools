(function(global) {
  ...
  var moduleId = path.indexOf('CTLSi-WB') >= 0 ? 'wb' :
    (path.indexOf('CTLSi-FUEL') >= 0 ? 'fuel' :
      (path.indexOf('CTLSi-TOL') >= 0 ? 'tol' : ''));
  if (!moduleId) return;

  var key = 'ctlsi_tool_draft_v1_' + moduleId;
  ...
  global.CTLSiToolDraftStore = Object.freeze({ save:save, restore:restore, moduleId:moduleId });
})(window);
