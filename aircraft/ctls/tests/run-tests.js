...
function createTolHarness() {
  const scriptPath = path.join(ROOT, 'CTLS-TOL', 'js', 'script.js');
...
function createWbHarness() {
  const htmlPath = path.join(ROOT, 'CTLS-WB', 'index.html');
  const html = fs.readFileSync(htmlPath, 'utf8');
  const match = html.match(/<script>([\s\S]*?)<\/script>/);
  assert.ok(match, 'CTLS-WB inline script not found');
...
function testFuelPlanner() {
  const dataPath = path.join(ROOT, 'CTLS-FUEL', 'data', 'pohData.js');
  const scriptPath = path.join(ROOT, 'CTLS-FUEL', 'js', 'script.js');
  const window = {};
  ...
  assert.equal(window.POH_DATA.FUEL_DENSITY_KG_PER_L, 0.72);
  assert.equal(window.POH_DATA.TANK_CAPACITY_L, 126);
...
function testFlightPlanStore() {
  ...
  vm.createContext(context);
  vm.runInContext(source, context, { filename: scriptPath });
  const api = window.CTLSFlightPlanStore;
...
function testToolDraftStore() {
  const scriptPath = path.join(ROOT, 'CTLS-common', 'js', 'tool-draft-store.js');
  const source = fs.readFileSync(scriptPath, 'utf8');
  const store = new Map([[
    'ctls_tool_draft_v1_wb',
  ...
  const window = {
    location: { pathname:'/CTLS-WB/index.html' }, document,
    localStorage: {
      getItem: key => store.has(key) ? store.get(key) : null,
      setItem: (key, value) => store.set(key, String(value))
    },
  ...
  fields[0].value = '180';
  listeners.input();
  assert.equal(JSON.parse(store.get('ctls_tool_draft_v1_wb')).values.w_crew, '180');
}

function testChecklistData() {
  const dataPath = path.join(ROOT, 'CTLS-CHECK', 'data', 'checklistData.js');
  const window = {};
  ...
  assert.ok(data.categories.every(category => category.groups.length > 0));
  const total = data.categories.reduce((sum, category) =>
    sum + category.groups.reduce((groupSum, group) => groupSum + group.items.length, 0), 0);
  assert.ok(total >= 200, `expected at least 200 POH-derived checklist items, got ${total}`);
  const emergency = data.categories.find(category => category.id === 'emergency');
  const content = JSON.stringify(data);
  assert.match(data.meta.source, /飞行检查单A5（定稿）/);
  const external = data.categories.find(category => category.id === 'external');
  const normal = data.categories.find(category => category.id === 'normal');
  assert.equal(external.groups.reduce((sum, group) => sum + group.items.length, 0), 56);
  assert.equal(normal.groups.reduce((sum, group) => sum + group.items.length, 0), 82);
  assert.match(content, /转速下降最多300 RPM/);
  assert.match(content, /最多120 RPM/);
  assert.match(content, /发电机\/主电瓶断电器/);
  assert.match(content, /低于250 m（750 ft）不得转回机场/);
  assert.match(content, /襟翼断路器.*拉出约3秒后复位/);
  assert.match(content, /不超过100 KIAS/);
  assert.doesNotMatch(content, /100-200 m/);
  assert.match(data.meta.applicability, /912 ULS/);
}
...
const tests = [
  ...
  ['Checklist structure and source warnings', testChecklistData]
];
